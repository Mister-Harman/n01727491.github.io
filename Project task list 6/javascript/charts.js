    ...
    try {
        ...
        songs.forEach((song, index) => {
            ...
            const title = `<div class="details">
                ...
                <strong>Release Date:</strong> ${song['im:releaseDate'] ? song['im:releaseDate'].label : 'N/A'}<br>
            </div>`;
            const previewLink = song.link[0].attributes.href ? `<a class="preview" href="${song.link[0].attributes.href}" target="_blank">Listen</a>` : '';
            
            songElement.innerHTML = `<span class="rank">${index + 1}.</span> ${coverImg} ${title} ${previewLink}`;
